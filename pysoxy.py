...
logging.basicConfig(filename='/pysoxy.log', level=logging.INFO)
#
# Configuration
#
MAX_THREADS = 200
...
def error(msg="", err=None):
    ...
    if msg:
        traceback.print_exc()
        logging.error("{} - Code: {}, Message: {}".format(msg, str(err[0]), err[1]))
    else:
        traceback.print_exc()
...
def bind_port(sock):
    ...
    """
    try:
        logging.info('Bind {}'.format(str(LOCAL_PORT)))
...
def exit_handler(signum, frame):
    ...
    logging.warning('Signal handler called with signal', signum)
...
def main():
    ...
    logging.info(f"Opened new socket for pysoxy")
